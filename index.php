...
<div data-role="page" id="pageone">
    ...
    if($MysqlStatement_select_data != null){
        while ($user_data = $MysqlStatement_select_data->fetchArray()) {
            ...
            echo "<label for=\"slider-fill\">Rating (User):</label>";
            echo "<input onChange='updateData(".$user_data['userdata_id']. ");' id='".$user_data['userdata_id']."' data-highlight=\"true\" type='range' name='slider' value='".$user_data['rating']."' min='0' max='5'>";
        }
    }
    ?>
    </div>
...
<script>
    function updateData(id){
        //my id
        console.log("Slider Changed:" + id);

        //get the data
        var slider_value = $('#'+id).val();
        console.log("Slider Value:" + slider_value);

        $.post( "PHP/updatedata.php",{ dataid: id, inputdata: slider_value }, function(data) {
